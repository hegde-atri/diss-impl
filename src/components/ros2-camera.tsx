...
export default function ROS2Camera({
  ...
  const handleImageError = () => {
    if (isServerRunning && isStreamEnabled) {
      setStreamError(
        'Failed to load video stream. The server is running but the stream could not be loaded.'
      );
    }
    setIsLoading(false);
  };
        ...
        >
          {isLoading && !isServerRunning ? (
            <div className='flex h-full w-full flex-col items-center justify-center'>
              ...
              <p className='mt-2 text-center'>Processing request...</p>
            </div>
          ) : isLoading && isServerRunning ? (
            <div className='flex h-full w-full flex-col items-center justify-center'>
              <Skeleton className='h-20 w-20 rounded-full' />
              <p className='mt-2 text-center'>Loading stream...</p>
            </div>
          ) : !robotPaired ? (
            <div className='flex h-full w-full items-center justify-center'>
              <p className='text-center'>
                Robot not connected. Please pair your robot first.
              </p>
            </div>
          ) : streamError ? (
            <div className='flex h-full w-full items-center justify-center'>
            ...
            <div className='flex h-full w-full items-center justify-center'>
              <p className='text-center'>
                Toggle the switch above to start the video server.
              </p>
            </div>
          ) : !isStreamEnabled ? (
            <div className='flex h-full w-full items-center justify-center'>
              <p className='text-center'>
                Stream is disabled. Toggle the switch to enable.
              </p>
            </div>
          ) : (
            <iframe
